...
# Запуск всех сервисов (включая PostGIS)
docker-compose up -d
```

> ⚠️ **Примечание:** `docker-compose.dev.yml` был перемещен в архив как устаревший. Основной `docker-compose.yml` уже содержит PostGIS.
...
Убедитесь, что конфигурация соответствует `docker-compose.yml`:
...
| Параметр | Значение |
|----------|----------|
| **Образ PostgreSQL** | postgis/postgis:15-3.4-alpine |
| **PostGIS** | ✅ Да |
| **Порт PostgreSQL** | 5433:5432 |
| **POSTGRES_DB** | service_db |
| **POSTGRES_USER** | service_user |
| **POSTGRES_PASSWORD** | service_password |
| **Контейнер** | service_postgres |
