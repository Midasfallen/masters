...
const CATALOG_DATA: CatalogCategory[] = [
  ...
  },
  {
    ...
    subcategories: [
      ...
      {
        name: 'Остеопатия и мануальная терапия',
        services: [
          'Остеопатия',
          'Мануальная терапия',
...
async function seedCatalog() {
  ...
  // Используем TreeRepository для автоматического заполнения categories_closure
  ...
  for (let rootIndex = 0; rootIndex < CATALOG_DATA.length; rootIndex++) {
    ...
    await translationRepository.save(
      translationRepository.create({
        ...
        language: 'ru',
        name: rootCat.name,
        description: `${rootCat.name} - категория услуг`,
    ...
    for (let subIndex = 0; subIndex < rootCat.subcategories.length; subIndex++) {
      ...
      await translationRepository.save(
        translationRepository.create({
          ...
          language: 'ru',
          name: subCat.name,
          description: `${subCat.name} - подкатегория в разделе ${rootCat.name}`,
