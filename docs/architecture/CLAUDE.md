...
### 9. Flutter - всегда проверяй пустые строки перед обращением к индексу
```dart
...
// ✅ Правильно - альтернативный вариант
Text(
  (user.firstName.isEmpty ? user.email : user.firstName)[0].toUpperCase(),
)

// ❌ Неправильно - может вызвать RangeError
Text(user.firstName[0].toUpperCase())  // Если firstName пустой!

// ✅ Правильно - для nullable строк
Text(
...
### v3.1 (20 января 2026) - Bug Fixes & Stability
- 🐛 Исправлена ошибка null pointer в ProfileScreen (profile_screen.dart:128)
  - Добавлена проверка user.firstName.isNotEmpty перед обращением к [0]
  - Fallback на user.email[0] если firstName пустой
  - Решает проблему загрузки профиля после входа
- 📝 Обновлен CLAUDE.md с версией 3.1
- 🔍 Добавлены best practices для работы с nullable/empty строками
