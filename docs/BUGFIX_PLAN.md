...
**Что нужно проверить:**
1. Проверить настройки `@OneToMany` в `Post` entity - есть ли `cascade: true`
2. Проверить метод `create()` в `posts.service.ts` - сохраняются ли медиа дважды
3. Если каскадное сохранение включено - убрать ручное сохранение медиа
4. Если каскадное сохранение отключено - проверить, что медиа сохраняются корректно

**Файлы для изменения:**
- `backend/src/modules/posts/posts.service.ts` - метод `create()` (строки 66-73)
- `backend/src/modules/posts/entities/post.entity.ts` - настройки `@OneToMany` для `media`
