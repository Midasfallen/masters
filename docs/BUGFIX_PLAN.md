...
**Рекомендации:**
- Добавить в блок «Auth» задачу по интеграции `shared_preferences` или улучшить использование `FlutterSecureStorage`
- При старте приложения в `main.dart` или через `authProvider` нужно проверять наличие токена
- Если токен есть - автоматически залогинивать пользователя без ввода пароля
- Убедиться, что `SplashScreen` дожидается загрузки `AuthNotifier` перед навигацией

**Файлы для изменения:**
- `frontend/lib/features/auth/screens/splash_screen.dart` - улучшить логику проверки, использовать `ref.watch()` или явное ожидание
- `frontend/lib/core/providers/api/auth_provider.dart` - проверить работу auto-login, убедиться что токен читается из storage
- `frontend/lib/core/repositories/auth_repository.dart` - проверить сохранение токенов, убедиться что `FlutterSecureStorage` работает на Web
- `frontend/lib/main.dart` - добавить проверку токена при старте (опционально)
