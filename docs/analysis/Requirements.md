...
> **Обновлено для v2.0:** Добавлены требования для социальной платформы
> **Статус:** Требования реализованы в MVP v2.0.
...
| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-110 | Клиент может оформить премиум-подписку | Must | MVP |
| FR-111 | Премиум-клиент не видит рекламу | Must | MVP |
| FR-112 | Премиум-клиент получает приоритетную поддержку | Should | MVP |
| FR-113 | Мастер может оформить премиум-подписку | Must | MVP |
| FR-114 | Премиум-мастер выше в результатах поиска | Must | MVP |
| FR-115 | Премиум-мастер выделен визуально | Must | MVP |
| FR-116 | Премиум-мастер получает расширенную статистику | Should | MVP |
| FR-117 | Мастер может пройти KYC верификацию | Should | v1.0 |
| FR-118 | Верифицированный мастер получает галочку | Should | v1.0 |
| FR-119 | Премиум-клиент получает скидку 5% (через платформу) | Must | v1.0 |
| FR-120 | Пользователь может пригласить друга по реферальному коду | Must | MVP |
| FR-121 | За приглашение друга дается бонус | Must | MVP |

### 1.10 Админ-панель

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-130 | Админ может просматривать дашборд с метриками | Must | MVP |
| FR-131 | Админ может модерировать контент (посты, отзывы) | Must | MVP |
| FR-132 | Админ может блокировать пользователей | Must | MVP |
| FR-133 | Админ может управлять каталогом услуг | Must | MVP |
| FR-134 | Админ может отвечать в техподдержке через чат | Should | MVP |
| FR-135 | Админ может просматривать детальную информацию о пользователях | Must | MVP |

### 1.11 Лента контента и посты (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-140 | Главный экран должен показывать ленту контента (Feed) | Must | Planned |
| FR-141 | Лента должна отображать посты в 3-колоночной сетке | Must | Planned |
| FR-142 | Клик на пост должен открывать полноэкранный просмотр | Must | Planned |
| FR-143 | В полноэкранном режиме можно свайпать между постами | Must | Planned |
| FR-144 | Лента должна иметь 3 вкладки: Рекомендации / Подписки / Рядом | Must | Planned |
| FR-145 | Алгоритм "Рекомендации" основан на геолокации (радиус 20км по умолчанию) | Must | Planned |
| FR-146 | Пользователь может настроить радиус: 5, 10, 20, 50, 100 км | Must | Planned |
| FR-147 | Мастер может создавать посты с фото (до 10) или видео (до 15 сек) | Must | Planned |
| FR-148 | Пост может содержать описание (до 500 символов) | Must | Planned |
| FR-149 | Мастер может упоминать (@) только обслуженных клиентов | Must | Planned |
| FR-150 | Посты показывают счетчики: лайки, комментарии, просмотры | Must | Planned |
| FR-151 | Лента должна поддерживать infinite scroll | Must | Planned |
| FR-152 | Фильтры ленты: категория, рейтинг, верифицированные, доступные слоты | Should | Planned |

### 1.12 Социальные функции (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-160 | Пользователь может лайкать посты (не анонимно) | Must | Planned |
| FR-161 | Пользователь может видеть список лайкнувших | Should | Planned |
| FR-162 | Пользователь может комментировать посты | Must | Planned |
| FR-163 | Комментарии поддерживают вложенные ответы (1 уровень) | Should | Planned |
| FR-164 | Пользователь может делать репосты в свою ленту | Should | Planned |
| FR-165 | Пользователь может поделиться постом/профилем в соцсетях | Should | Planned |
| FR-166 | Модерация: жалобы на спам/неприемлемый контент | Must | Planned |

### 1.13 Друзья (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-170 | Пользователь может отправить запрос в друзья | Must | Planned |
| FR-171 | Получатель может принять/отклонить/заблокировать запрос | Must | Planned |
| FR-172 | Запрос имеет статусы: pending, accepted, rejected, blocked | Must | Planned |
| FR-173 | Дружба двусторонняя (требует согласия обеих сторон) | Must | Planned |
| FR-174 | Пользователь может удалить из друзей | Must | Planned |
| FR-175 | Профиль показывает счетчик друзей | Must | Planned |
| FR-176 | Посты друзей показываются в приоритете в ленте | Should | Planned |
| FR-177 | Друзья могут видеть статус онлайн (опционально) | Could | Planned |

### 1.14 Подписки (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-180 | Пользователь может подписаться на мастера одним кликом | Must | Planned |
| FR-181 | Подписка не требует согласия мастера (односторонняя) | Must | Planned |
| FR-182 | Пользователь может отписаться в любой момент | Must | Planned |
| FR-183 | Профиль показывает счетчики: подписчики (followers) и подписки (following) | Must | Planned |
| FR-184 | Посты мастеров, на которых подписан, появляются в ленте "Подписки" | Must | Planned |
| FR-185 | Пользователь может получать уведомления о новых постах (опционально) | Should | Planned |

### 1.15 Обязательные отзывы (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-190 | После выполнения услуги клиент и мастер ОБЯЗАНЫ оставить отзывы | Must | Planned |
| FR-191 | Без отзыва блокируется возможность новых записей | Must | Planned |
| FR-192 | Клиент не может записаться к другим мастерам без отзыва | Must | Planned |
| FR-193 | Мастер не может принимать новые записи без отзыва о клиенте | Must | Planned |
| FR-194 | Система отправляет напоминания: через 24ч, 48ч, 72ч после услуги | Must | Planned |
| FR-195 | Отзыв клиента: оценка 1-5, текст (мин 20 символов), фото (опц) | Must | Planned |
| FR-196 | Отзыв мастера: оценка 1-5, текст (опц), виден только мастерам | Must | Planned |

### 1.16 Автопредложения (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-200 | Премиум-мастер может настроить критерии автопредложений | Should | Planned |
| FR-201 | Критерии: категория услуг, радиус, время, рейтинг клиента | Should | Planned |
| FR-202 | Система автоматически находит подходящих клиентов | Should | Planned |
| FR-203 | Мастер видит список потенциальных клиентов | Should | Planned |
| FR-204 | Мастер может отправить предложение записи клиенту | Should | Planned |
| FR-205 | Клиент получает push о предложении и может принять/отклонить | Should | Planned |
| FR-206 | Лимиты: 10 предложений/день (базовый), 50 предложений/день (премиум+) | Should | Planned |
| FR-207 | Доступно только для премиум-мастеров | Should | Planned |

### 1.17 Чаты (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-210 | "Чаты" заменяют "Избранное" в навигации | Must | Planned |
| FR-211 | Список чатов показывает последнее сообщение и время | Must | Planned |
| FR-212 | Непрочитанные сообщения показываются счетчиком | Must | Planned |
| FR-213 | Чат работает в реальном времени (WebSocket) | Must | Planned |
| FR-214 | Поддержка текстовых сообщений | Must | Planned |
| FR-215 | Поддержка отправки фото | Must | Planned |
| FR-216 | Данные пользователей скрыты до подтверждения записи | Must | Planned |

### 1.18 Навигация (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-220 | Bottom Navigation имеет 5 вкладок: Feed, Search, Chats, Bookings, Profile | Must | Planned |
| FR-221 | Feed - главный экран (вместо Home) | Must | Planned |
| FR-222 | Экран выбора роли удален из потока регистрации | Must | Planned |
| FR-223 | Все пользователи регистрируются как клиенты по умолчанию | Must | Planned |

### 1.19 Создание профиля мастера (v2.0)

| ID | Требование | Приоритет | Статус |
|----|------------|-----------|---------|
| FR-230 | Создание профиля мастера - 5-шаговый процесс | Must | Planned |
| FR-231 | Шаг 1: Выбор категорий и добавление услуг | Must | Planned |
| FR-232 | Шаг 2: Указание цен и длительности услуг | Must | Planned |
| FR-233 | Шаг 3: Загрузка портфолио (минимум 3 фото) | Must | Planned |
| FR-234 | Шаг 4: Настройка географии работы (локация, выезд) | Must | Planned |
| FR-235 | Шаг 5: Настройка расписания (7 дней недели) | Must | Planned |
| FR-236 | После завершения 5 шагов: is_master = true | Must | Planned |
| FR-237 | Пользователь может быть клиентом и мастером одновременно | Must | Planned |
...
- [BRD](../business/BRD.md) - Бизнес-требования (актуально)
- [User Stories](../business/UserStories.md) - Пользовательские истории (актуально)

Подробная техническая спецификация и дорожная карта перенесены в архив:
- Technical Specification, Roadmap — см. `archive/docs-temporary/`

---

**Статус:** Утверждено и реализовано в рамках MVP v2.0
**Последнее обновление:** 5 февраля 2026
**Версия:** 2.0

**Добавлено в v2.0:** 98 новых FR (FR-140 до FR-237) для социальной платформы
