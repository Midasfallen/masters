...
Проект содержит два Docker Compose файла для разных сценариев использования:

| Файл | Назначение | PostGIS | Рекомендуется для |
|------|------------|---------|-------------------|
| `docker-compose.yml` | Production / Default | Нет | CI/CD, staging, production |
| `docker-compose.dev.yml` | Development | Да | Разработка с геолокацией |
