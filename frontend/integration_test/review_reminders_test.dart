  ...
  group('Review Reminders System E2E Tests', () {
    testWidgets('Отображение диалога с неотзывленными бронированиями',
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        debugPrint('✅ Диалог с неотзывленными бронированиями отображается корректно');
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для тестирования');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        if (gracePeriodButton.evaluate().isNotEmpty) {
          ...
          debugPrint('✅ Grace period использован успешно');
        } else {
          debugPrint('⚠️ Кнопка "Напомнить позже" не найдена (возможно уже использована)');
        }
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для тестирования grace period');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        if (skipButton.evaluate().isNotEmpty) {
          ...
          debugPrint('✅ Обычный skip выполнен');
        } else {
          debugPrint('⚠️ Кнопка "Пропустить" не найдена (может быть доступна только после 3+ напоминаний)');
        }
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для тестирования skip');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        if (skipButton.evaluate().isNotEmpty) {
          debugPrint('✅ Кнопка "Пропустить" доступна (>=3 напоминаний)');
        } else {
          debugPrint('✅ Кнопка "Пропустить" скрыта (<3 напоминаний)');
        }
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для проверки логики кнопок');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        debugPrint('✅ Навигация к экрану оставления отзывов выполнена');
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для тестирования навигации');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        if (graceInfoFinder.evaluate().isNotEmpty) {
          ...
          debugPrint('✅ Информация о grace period отображается');
        } else {
          debugPrint('ℹ️ Grace period уже использован или не доступен');
        }
        ...
        if (usedGraceWarning.evaluate().isNotEmpty) {
          debugPrint('✅ Предупреждение о пропущенных напоминаниях отображается');
        }
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для проверки информации о grace period');
      }
    });
    ...
    testWidgets(
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        debugPrint('✅ Badge счетчика напоминаний отображается');
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для проверки badge');
      }
    });
      ...
      if (dialogFinder.evaluate().isNotEmpty) {
        ...
        if (skipButton.evaluate().isNotEmpty) {
          ...
          if (errorSnackBar.evaluate().isNotEmpty) {
            debugPrint('✅ Ошибка API обработана корректно');
          } else {
            debugPrint('✅ API запрос выполнен успешно');
          }
        }
      } else {
        debugPrint('⏩ Нет неотзывленных бронирований для тестирования обработки ошибок');
      }
    });
  });
}
...
Future<void> _loginAsClient(WidgetTester tester) async {
  ...
  if (emailField.evaluate().isNotEmpty) {
    ...
    debugPrint('✅ Вход выполнен как клиент');
  } else {
    debugPrint('⚠️ Уже залогинен или экран логина не найден');
  }
}
